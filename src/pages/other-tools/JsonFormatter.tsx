...
function JsonFormatter() {
  ...
  const handleFormat = () => {
    try {
      ...
      setOutput(formatted);
      setStatus(
        t("otherTools.jsonFormatter.success", {
          action: t("otherTools.jsonFormatter.format")
        })
      );
    } catch (error) {
      const errorMessage =
  ...
  const handleMinify = () => {
    try {
      ...
      setStatus(
        t("otherTools.jsonFormatter.success", {
          action: t("otherTools.jsonFormatter.minify")
        })
      );
    } catch (error) {
      const errorMessage =
      ...
      <div className="tool-card">
        ...
        <div style={{ display: "flex", gap: "1rem", marginTop: "1rem" }}>
          <button onClick={handleFormat}>
            {t("otherTools.jsonFormatter.format")}
          </button>
          <button onClick={handleMinify}>
            {t("otherTools.jsonFormatter.minify")}
          </button>
        ...
        <div className="info-box" style={{ marginTop: "2rem" }}>
          ...
          <ul>
            <li>
              <strong>格式化</strong>：美化 JSON，添加縮排和換行
            </li>
            <li>
              <strong>壓縮</strong>：移除所有空白字符，最小化 JSON 大小
            </li>
            <li>
              <strong>驗證</strong>：自動檢測 JSON 語法錯誤
            </li>
